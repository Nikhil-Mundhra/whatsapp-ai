...
export async function POST(_req, { params }) {
  ...
  const res = await fetch(`${BRIDGE_URL}/api/connections/${params.hash}`, {
    method: "POST",
  ...
  await updateConnection(params.hash, { status: "pairing" });
...
export async function GET(_req, { params }) {
  ...
  const res = await fetch(`${BRIDGE_URL}/api/connections/${params.hash}/qr`, {
    cache: "no-store",
